...
300mm+（`fBase ≥ 300` 或 `fEff ≥ 300`）外壳与镜组按参考剖面图**像素实测**的归一化轴向比例 `u = z / L`（`u=0` 卡口，`u=1` 前口）摆放。侧影为**连续收锥**（非台阶尖口），半径 `r(u)/D_front` 控制点：

| u | 0 | 0.1 | 0.3 | 0.5 | 0.65 | 0.8 | 0.9 | 1.0 |
|---|---|-----|-----|-----|------|-----|-----|-----|
| r/D_front | 0.175 | 0.25 | 0.335 | 0.335 | 0.40 | 0.445 | 0.50 | 0.48 |

**内部镜组中心**（实测色块 u）：前大弯月 **0.95**；黄胶合三片 **0.47 / 0.50 / 0.53**；橙弯月 **0.42**；中继清片 **0.36 / 0.39**；绿后组 **0.12 / 0.16 / 0.20**；后端平板 **0.08**。镜组口径随 `profileRadius(u)` 内收，保证剖面「流畅」。
